		...
		openButton = new Button("Open",0,0){
			...
			public void click(DialogBox parent){
				...
				if(onesel){
					...
					try (final BufferedReader br = new BufferedReader(new InputStreamReader(new FileInputStream("data/saves/" + saveGame.getName())))) {
	...
	public void update(){
		...
		files = new SingleTextSelectionElement[saves.length];
		    for (int i = 0; i < saves.length; i++) {
		      if (saves[i].isFile()) {
		        files[i] = new SingleTextSelectionElement(-73, -82 + (16*i),128 ,new BitmapString(saves[i].getName(), RenderEngine.font, 0.5f));
		      } 
		    }
	}
